...
unsigned int handlePacket(){
	 ...
	 switch(header.commandType){
		  case CONNACK_COMMAND:
				Serial.printlnf("[IN] CONNACK %u, %u", header.controlFlags, header.length);
		  ...
		  case PUBCOMP_COMMAND:
				Serial.printlnf("[IN] PUB* %u, %u, %u", header.commandType, header.controlFlags, header.length);
				PubackPacket::decode();
		  break;
		  
		  case SUBACK_COMMAND:
				Serial.printlnf("[IN] SUBACK %u, %u", header.controlFlags, header.length);
				SubackPacket::decode();
		  break;
		  ...
		  case PUBLISH_COMMAND: {
				...
				Serial.printlnf("[IN] PUBLISH: Message received in topic '%s': %s", 
		  ...
		  default:
				Serial.printlnf("[IN] ??? %u, %u, %u", header.commandType, header.controlFlags, header.length);
...
void connect(String clientID, unsigned int keepAlive = 60){
	 ...
	 Serial.printlnf("[OUT] CONNECT: Connecting with client ID '%s' and a keep alive interval of seconds.", 
		  clientID.c_str(), keepAlive
	 );
...
void pingreq(){
	 ...
	 Serial.printlnf("[OUT] PINGREQ %u, %u", packet.header.controlFlags, packet.header.length);
...
void disconnect(){
	 ...
	 Serial.printlnf("[OUT] DISCONNECT %u, %u", disconnectPacket.header.controlFlags, disconnectPacket.header.length);
...
uint16_t publish(String topicName, String payload){
	 ...
	 Serial.printlnf("[OUT] PUBLISH: Message sent in topic '%s': %s", 
		  topicName.c_str(), payload.c_str()
	 );
...
void subscribe(String topicName){
	 ...
	 Serial.printlnf("[OUT] SUBSCRIBE: Subscribed to topic '%s'.", topicName.c_str());
...
String waveTopicName = "SIT210/jess/wave";
String patTopicName = "SIT210/jess/pat";
